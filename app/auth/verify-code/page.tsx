...
function VerifyCodeForm() {
  ...
  const handleSubmit = async (codeValue?: string) => {
    ...
    if (finalCode.length !== 6) {
      setError(t?.verifyCode?.errors?.invalidCode ?? "Введите 6-значный код");
      return;
    }
    ...
    <div className="min-h-screen bg-[#0a1f1a] flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#0f2d26] border border-emerald-500/30 rounded-2xl p-8">
          ...
          <div className="text-center mb-6">
            ...
            <h1 className="text-3xl font-bold text-white mb-2">
              {t?.verifyCode?.title ?? "Введите код"}
            </h1>
            <p className="text-gray-400">
              {t?.verifyCode?.description ?? "Мы отправили 6-значный код на"}
            </p>
          ...
          >
            {loading
              ? (t?.verifyCode?.checking ?? "Проверка...")
              : (t?.verifyCode?.confirmButton ?? "Подтвердить")}
          </button>

          {/* Resend Code */}
          <div className="text-center">
            <p className="text-gray-400 text-sm mb-2">
              {t?.verifyCode?.noCodeReceived ?? "Не получили код?"}
            </p>
            <button
            ...
            >
              {resending
                ? (t?.verifyCode?.resending ?? "Отправка...")
                : countdown > 0
                  ? (
                      t?.verifyCode?.resendTimer ??
                      "Отправить повторно ({countdown}с)"
                    ).replace("{countdown}", String(countdown))
                  : (t?.verifyCode?.resend ?? "Отправить повторно")}
            </button>
          </div>
        </div>
        ...
        <div className="mt-6 text-center">
          ...
          >
            <ArrowLeft size={16} />
            {t?.verifyCode?.backToLogin ?? "Вернуться к входу"}
          </Link>
        </div>
      </div>
    </div>
